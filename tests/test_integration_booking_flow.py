...
class TestToolResultsIncludeDuration:
    """
    Verify that execute_tool_call returns duration_minutes and duration_label
    for get_next_available, check_availability, and search_availability.
    """

    @pytest.fixture
    def services_no_workers(self):
        monday = next_monday()
        cal = make_mock_calendar(existing_bookings=[], biz_start=8, biz_end=17)
        db = make_mock_db(workers=None)
        return {
            'google_calendar': cal,
            'db': db,
            'company_id': 1,
    ...
    @pytest.mark.parametrize("svc_key,expected_mins", [
        ('short_1h', 60),
        ('short_2h', 120),
        ('full_day', 1440),
        ('week', 10080),
    ...
    def test_get_next_available_returns_duration(self, services_no_workers, svc_key, expected_mins):
        from src.services.calendar_tools import execute_tool_call
        svc = SERVICE_CONFIGS[svc_key]
        with patch('src.services.calendar_tools.match_service') as mock_match:
            mock_match.return_value = {'service': svc, 'confidence': 1.0}
            with patch('src.utils.config.config', _mock_config()):
                result = execute_tool_call('get_next_available',
                    {'job_description': svc['name']}, services_no_workers)

        assert result['success'] is True
        assert result['duration_minutes'] == expected_mins
        assert 'duration_label' in result
        assert len(result['duration_label']) > 0
    ...
    def test_check_availability_returns_duration(self, services_no_workers, svc_key, expected_mins):
        from src.services.calendar_tools import execute_tool_call
        svc = SERVICE_CONFIGS[svc_key]
        monday = next_monday()
        with patch('src.services.calendar_tools.match_service') as mock_match:
            mock_match.return_value = {'service': svc, 'confidence': 1.0}
            with patch('src.utils.config.config', _mock_config()):
                with patch('src.utils.date_parser.parse_datetime') as mock_parse:
                    mock_parse.return_value = monday
                    result = execute_tool_call('check_availability',
                        {'start_date': monday.strftime('%Y-%m-%d'),
                         'job_description': svc['name']},
                        services_no_workers)
...
class TestBusinessHoursPropagation:
    ...
    def test_db_wrapper_uses_company_hours(self):
        ...
        from src.services.db_postgres_wrapper import PostgreSQLDatabaseWrapper
        wrapper = PostgreSQLDatabaseWrapper.__new__(PostgreSQLDatabaseWrapper)
        monday = next_monday(hour=8)

        # Full-day job with 8-17 hours
        end = wrapper._calculate_job_end_time(monday, 1440, biz_start_hour=8, biz_end_hour=17)
        assert end.hour == 17
        assert end.minute == 15  # 15 min buffer in wrapper
...
class TestExecuteToolCallFlow:
    """
    End-to-end tests through execute_tool_call verifying the complete
    flow from tool invocation to result structure.
    """

    def _run_get_next_available(self, svc_config, existing_bookings=None,
                                 workers=None, biz_start=8, biz_end=17):
        from src.services.calendar_tools import execute_tool_call
        if workers:
            db = make_mock_db(existing_bookings=existing_bookings or [], workers=workers)
        else:
            db = make_mock_db(existing_bookings=None, workers=None)
        cal = make_mock_calendar(existing_bookings=existing_bookings or [],
                                 biz_start=biz_start, biz_end=biz_end)
        services = {'google_calendar': cal, 'db': db, 'company_id': 1}

        mock_cfg = _mock_config()
        mock_cfg.get_business_hours.return_value = {'start': biz_start, 'end': biz_end}

        with patch('src.services.calendar_tools.match_service') as mock_match:
            mock_match.return_value = {'service': svc_config, 'confidence': 1.0}
            with patch('src.utils.config.config', mock_cfg):
                return execute_tool_call('get_next_available',
                    {'job_description': svc_config['name']}, services)

    def test_1h_job_returns_time_slots(self):
        result = self._run_get_next_available(SERVICE_CONFIGS['short_1h'])
    ...
    def test_full_day_job_returns_day_slots(self):
        result = self._run_get_next_available(SERVICE_CONFIGS['full_day'])
        assert result['success']
        assert result.get('is_full_day_service') is True
        assert result['duration_minutes'] == 1440
        assert result['duration_label'] == 'a full day'

    def test_week_job_returns_day_slots(self):
        result = self._run_get_next_available(SERVICE_CONFIGS['week'])
        assert result['success']
        assert result.get('is_full_day_service') is True
        assert result['duration_minutes'] == 10080
        assert result['duration_label'] == 'about a week'

    def test_month_job_returns_day_slots(self):
        result = self._run_get_next_available(SERVICE_CONFIGS['month'])
        assert result['success']
        assert result.get('is_full_day_service') is True
        assert result['duration_minutes'] == 40320
        assert result['duration_label'] == 'about a month'

    def test_8am_business_hours_used(self):
        """Verify that 8am opening is reflected in available slots."""
        result = self._run_get_next_available(
            SERVICE_CONFIGS['short_1h'], biz_start=8, biz_end=17
        )
        assert result['success']
        if result.get('available_days'):
            first_slot = result['available_days'][0].get('first_slot', '')
            # First slot should be at 8:00 AM or 8:30 AM (not 9:00 AM)
            assert '08:' in first_slot or '09:' in first_slot  # Could be 8:00 or 8:30

    def test_natural_summary_present(self):
        result = self._run_get_next_available(SERVICE_CONFIGS['short_2h'])
        assert result['success']
        assert 'natural_summary' in result
        assert len(result['natural_summary']) > 0

    def test_voice_instruction_for_full_day(self):
        result = self._run_get_next_available(SERVICE_CONFIGS['full_day'])
        assert result['success']
        assert 'voice_instruction' in result
        assert 'DAY' in result['voice_instruction']
        assert 'time' not in result['voice_instruction'].lower() or 'don' in result['voice_instruction'].lower()


# ─── Prompt content tests ─────────────────────────────────────────────

class TestPromptContent:
    """Verify the prompt file has the correct instructions."""

    def test_prompt_has_all_duration_rule(self):
        with open('prompts/receptionist_prompt_fast.txt', 'r') as f:
            content = f.read()
        assert 'JOB DURATION RULE (ALL JOBS)' in content
        assert 'ALWAYS mention the job length BEFORE listing available start days/times' in content

    def test_prompt_has_short_job_example(self):
        with open('prompts/receptionist_prompt_fast.txt', 'r') as f:
            content = f.read()
        assert 'boiler repair' in content.lower() or 'Boiler' in content

    def test_prompt_has_full_day_example(self):
        with open('prompts/receptionist_prompt_fast.txt', 'r') as f:
            content = f.read()
        assert 'general service' in content.lower() or 'full day' in content.lower()

    def test_prompt_has_multi_day_example(self):
        with open('prompts/receptionist_prompt_fast.txt', 'r') as f:
            content = f.read()
        assert 'kitchen installation' in content.lower() or 'about a week' in content.lower()

    def test_full_day_section_no_duplicate_mention_rule(self):
        """The FULL-DAY JOBS section should NOT have the old 'ALWAYS mention' line
        since it's now in the general JOB DURATION RULE section."""
        with open('prompts/receptionist_prompt_fast.txt', 'r') as f:
            content = f.read()
        # Find the FULL-DAY JOBS section
        full_day_idx = content.index('FULL-DAY JOBS')
        general_q_idx = content.index('GENERAL QUESTIONS')
        full_day_section = content[full_day_idx:general_q_idx]
        assert 'ALWAYS mention the job length' not in full_day_section
